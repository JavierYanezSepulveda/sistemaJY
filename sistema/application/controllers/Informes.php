    ...
    if ($query1!=null) {
     ...
     if ($resultados != FALSE) {
        ...
        echo "<script> alert('No hay registros');</script>";
         redirect('Informes/informesCompras', 'refresh'); 
       }
  ...
  {   
    ...
    $data= array();
     ...
     $this->load->view('footer');
      ...
      if ($query1) {
       $resultados=$this->ventas_model->buscar($query1,$query2,$id_sucursal);
      if ($resultados != FALSE) {
        ...
        $productos1 = $this->ventas_model->obtener_todosproductos();
            $data= array("resultados"=> $resultados,
              ...
              "query1"=> $query1,"query2"=> $query2,"productos1"=> $productos1 );
       }else{
      echo "<script> alert('No hay registros');</script>";
      redirect('Informes/informesCompras', 'refresh'); 
       }
  ...
  {   
    ...
    $data= array();
     ...
     public function generar() {
        ...
        define ('PDF_HEADER_TITLE', 'Informe de ventas');
...
// Este método tiene varias opciones, consulte la documentación para más información.
   ...
   public function genera() {
        ...
        $pdf->SetSubject('Informe');
        $pdf->SetKeywords('guide');
        define ('PDF_HEADER_TITLE', 'Informe de Ordenes');
        define ('PDF_HEADER_LOGO', '');
...
// Establecemos el contenido para imprimir
       ...
       $html.="table,th {
        ...
        $html.= "</style>";
        $html.= "<h4>Cantidad de Ordenes realizadas: ".count($resultados)."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Desde&nbsp;" . date("d-m-Y ", strtotime($query1))."&nbsp;&nbsp;&nbsp;Hasta&nbsp;". date("d-m-Y ", strtotime($query2))."</h4>";
        $html.= "<table width='100%'>";
        ...
        foreach ($resultados as $fila) {
            ...
            foreach($ucc as $item1){
             if ($fila['ID_UCC']===$item1['ID_UCC'] && $item1['TIPO_MATERIAL']==='0') {
                $enseñanza =$fila['TOTAL']; 
                  }
           elseif($fila['ID_UCC']===$item1['ID_UCC'] &&$item1['TIPO_MATERIAL']==='1'){
       ...
       $sumando=0;
         ...
         $sumando1=0;
          if(is_array($resultados) && is_array($ucc)) {
         foreach($resultados as $item)
         foreach($ucc as $item1)
    if ($item['ID_UCC']===$item1['ID_UCC'] && $item1['TIPO_MATERIAL']==='1') {
            $sumando1 += $item['TOTAL'];
         }}
         ...
         foreach($resultados as $item)
               foreach($ucc as $item1)
     if ($item['ID_UCC']===$item1['ID_UCC'] && $item1['TIPO_MATERIAL']==='0'){ 
               $sumando += $item['TOTAL'];}}
           $html.= "<br>";
           $html.= "<br>";
           $html.= "<br>";
...
// Este método tiene varias opciones, consulte la documentación para más información.
  ...
  public function generarC() {
        ...
        $pdf->SetSubject('Informe');
        $pdf->SetKeywords('guide');
        define ('PDF_HEADER_TITLE', 'Informe de Compras');
        define ('PDF_HEADER_LOGO', '');
// datos por defecto de cabecera, se pueden modificar en el archivo tcpdf_config_alt.php de libraries/config
        $pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE , PDF_HEADER_STRING, array(0, 64, 255), array(0, 64, 128));
        $pdf->setFooterData($tc = array(0, 64, 0), $lc = array(0, 64, 128));
...
// Establecemos el contenido para imprimir
       ...
       $html.="table,th {
        ...
        $html.= "</style>";
        $html.= "<h4>Cantidad de Compras realizadas: ".count($resultados)."&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Desde&nbsp;" . date("d-m-Y ", strtotime($query1))."&nbsp;&nbsp;&nbsp;Hasta&nbsp;". date("d-m-Y ", strtotime($query2))."</h4>";
        $html.= "<table width='100%'>";
